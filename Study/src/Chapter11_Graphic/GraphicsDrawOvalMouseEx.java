...
public class GraphicsDrawOvalMouseEx extends JFrame {
	...
	class MyPanel extends JPanel {
		...
		public MyPanel() {
			...
			// listener를 아래 두 리스너로 공통으로 동록해야 한다
		...
		class MyMouseListener extends MouseAdapter { // 마우스 리스너
			...
			public void mousePressed(MouseEvent e) {
				start = e.getPoint(); // 마우스 포인터(시작점)을 start 객체에 저장
			}
			@Override
			public void mouseDragged(MouseEvent e) {
				end = e.getPoint(); // 마우스 포인터(끝점)을 end 객체에 저장
				repaint(); // 패널의 그리기 요청 >> repaint()가 호출되면 MyPanel의 paintComponent()가 호출된다. 여기서 start와 end 사이의 타원을 그린다.
			}
		}
		
		@Override
		protected void paintComponent(Graphics g) { // 패널에 타원 그리기
			super.paintComponent(g);
			if(start == null) { // 타원이 만들어지지 않았을 경우
				return;
			}
