...
public class CheckBoxItemEventEx extends JFrame {
	...
	public CheckBoxItemEventEx() {
		setTitle("페크박스와 ItemEvent 예제");
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		Container c = getContentPane();
		c.setLayout(new FlowLayout());
		
		c.add(new JLabel("사과 100원, 배 500, 체리 20000원"));
		
		// 3개의 체크박스 컴포넌트를 생성하고 ContentPane에 삽입, Item 리스너 등록
		MyItemListener listener = new MyItemListener();
		for (int i = 0; i < fruits.length; i++) {
			fruits[i] = new JCheckBox(names[i]); // names[i]의 문자열로 체크박스 생성
			fruits[i].setBorderPainted(true); // 체크박스의 외곽선이 보이도록 설정
			c.add(fruits[i]); // ContentPane에 체크박스 삽입
			fruits[i].addItemListener(listener); // 체크박스에 Item 리스너 등록
		}
		
		sumLabel = new JLabel("현재 0원입니다."); // 가격 합을 출력하는 레이블 생성
		c.add(sumLabel);
	...
	class MyItemListener implements ItemListener {
		...
		public void itemStateChanged(ItemEvent e) {
			...
			sumLabel.setText("현재 " + sum + "원입니다."); // 합 출력
		}
	}
